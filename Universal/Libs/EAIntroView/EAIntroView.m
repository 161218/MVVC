...
- (void)buildFooterView {
    ...
    self.pageControl.hidden = YES;//故意隐藏

    self.skipButton = [[UIButton alloc] initWithFrame:CGRectMake(self.scrollView.frame.size.width - 60, 20, 40, 40)];
   ...
   //[self.skipButton setTitle:@"跳过" forState:UIControlStateNormal];
    ...
    UIButton *lijitiyanbrn=[[UIButton alloc] initWithFrame:CGRectMake(self.scrollView.frame.size.width/2-60, self.scrollView.frame.size.height-120, 140, 40)];
    [lijitiyanbrn addTarget:self action:@selector(skipIntroduction) forControlEvents:UIControlEventTouchUpInside];
    lijitiyanbrn.backgroundColor=[UIColor clearColor];
    [self addSubview:lijitiyanbrn];
}
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView {
    ...
    if (page == 4) {
        ...
        self.skipButton = [[UIButton alloc] initWithFrame:CGRectMake(0, self.frame.size.height - 240, 320, 240)];
        [self.skipButton setTitleColor:[UIColor redColor] forState:UIControlStateNormal];
        self.skipButton.titleLabel.font = [UIFont systemFontOfSize:30];
        [self.skipButton setAutoresizingMask: UIViewAutoresizingFlexibleLeftMargin | UIViewAutoresizingFlexibleRightMargin];
